var pokemon = {
  ...
  printAllPokemonNamesToConsole : function(){
    _.each(pokemon.pokemonList, function(monster){console.log(monster.name);
    });
  },

  findPokemonByName : function(name){
    return _.find(pokemon.pokemonList, function(monster){ return monster.name === name; }
      );
  },

  // same as:
  // findPokemonByName : function(name){
  //   var thepokemoniwant = _.find(pokemon.pokemonList, function(monster){ return monster.name === name; 
  //   });
  //   return thepokemoniwant;
  // },

  findStrongestPokemon : function(){
    return _.max(pokemon.pokemonList, function(monster) { return parseInt(monster.stats.attack );
    });
  },
